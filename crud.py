...
def get_employer_by_email1(db: Session, email: str):
    return db.query(models.Employer).filter(models.Employer.email == email,models.Employer.status==1).first()
...
def get_posted_jobs(db:Session,emp_id:int):
    return db.query(models.Jobs.job_id,
                    models.Jobs.title,
                    models.Jobs.description,
                    models.Jobs.post,
                    models.Jobs.company_name,
                    models.Jobs.annual_salary_in_lakhs,
                    models.Jobs.job_location,
                    models.Jobs.apply_from,
                    models.Jobs.apply_to).filter(models.Jobs.posted_by==emp_id).all()
...
def get_cand_skills(db:Session,cand_id:int):
    return db.query(models.Skills).filter(models.Skills.cand_id==cand_id).first()

def update_cand_skills(db:Session,skill1:str,skill2:str,skill3:str,skill4:str,cand_id:int):
    db.query(models.Skills).filter(models.Skills.cand_id == cand_id). \
                update({models.Skills.skill1:skill1,
                        models.Skills.skill2:skill2,
                        models.Skills.skill3:skill3,
                        models.Skills.skill4:skill4},synchronize_session=False)
    db.commit()
    return True
...
def get_applied_jobs(db:Session,cand_id:int):
    ...
    return db.query(models.Jobs.job_id,
                    ...
                    models.Jobs.job_location,
                    models.Jobs.apply_from,
                    models.Jobs.apply_to).filter(models.Jobs.job_id.in_([f.job_id for f in db_apply])).all()
...
def get_admin(db: Session, userid: str,password:str):
    return db.query(models.Admin).filter(models.Admin.userid == userid,models.Admin.password==password).first()
